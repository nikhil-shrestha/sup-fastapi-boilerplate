...
def init_db(db: Session) -> None:
    ...
    account = crud.account.get_by_name(
        db, name=settings.SUPER_ADMIN_ACCOUNT_NAME
    )
    if not account:
        account_in = schemas.AccountCreate(
            name=settings.SUPER_ADMIN_ACCOUNT_NAME,
            description="superadmin account",
        )
        crud.account.create(db, obj_in=account_in)

    # Create 1st Superuser
    user = crud.user.get_by_email(db, email=settings.SUPER_ADMIN_EMAIL)
    if not user:
        account = crud.account.get_by_name(
            db, name=settings.SUPER_ADMIN_ACCOUNT_NAME
        )
        user_in = schemas.UserCreate(
            ...
            full_name=settings.SUPER_ADMIN_EMAIL,
